-- Goodwill Vietnam - Full Database Import (single file)
-- Safe to run on a fresh DB to create all core structures, triggers, views, and seed data.
...
-- Indexes (tables were dropped above so names are free)
...
INSERT INTO categories (category_id, name, description, icon, sort_order) VALUES
    ...
    (2, 'Electronics', 'Phones, laptops, devices', 'bi-laptop', 2),
    (3, 'Books', 'Books and documents', 'bi-book', 3),
    (4, 'Home', 'Household items', 'bi-house', 4),
    (5, 'Toys', 'Toys for children', 'bi-toy', 5),
...
-- Seed admin users (password hashes are placeholders)
...
INSERT INTO system_settings (setting_key, setting_value, description, type) VALUES
    ...
    ('site_description', 'Nền tảng thiện nguyện kết nối cộng đồng', 'Site description', 'string'),
...
SELECT 
    ...
    CASE 
        ...
        WHEN i.status = 'reserved' THEN 'Đã giữ'
...
SELECT 
    ...
    CASE 
        ...
        WHEN i.price_type = 'normal' THEN 'Giá thường'
